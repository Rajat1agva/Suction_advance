...
{
	PrintDrawingArea = ST7586_getReservedAreaMalloc(x,y,(strlen(character) * 6 * _size) + x, y + 9 * _size);
...
{
	PrintDrawingArea = ST7586_getReservedAreaMalloc(x, y, (strlen_P(character) * 6 * _size) + x, y + 9 * _size);
	uint8_t _temp = ST7586_BufferEnabled;
	ST7586_BufferEnabled = 2;
...
void glcd_char(uint16_t x, uint16_t y, char character, uint8_t fgcolor, uint8_t bgcolor, uint8_t size)
{
	...
	{ // Char bitmap = 5 columns
		char line = pgm_read_byte(&(font[character * 8 + i]));
...
{	
#ifdef ST7586_H_
	ST7586_DrawingArea = ST7586_getReservedAreaMalloc(x,y,(strlen(character)*9)+x,y+9);
#endif
	while(*character)
	{
		glcd_char(x, y, *character++, _fgcolor, _bgcolor, _size);
...
{
	PrintDrawingArea = ST7586_getReservedAreaMalloc(x,y,(strlen(character) * 7 * _size) + x, y + 8 * _size);
	uint8_t _temp = ST7586_BufferEnabled;
	ST7586_BufferEnabled = 2;
	
	while(*character)
	{
		//glcd_draw_line(x-1, y, x + _size - 1, y, _bgcolor);
		glcd_char(x, y, *character, _fgcolor, _bgcolor, _size);
		if (*character++)
		{
			x += _size * 7;
		}
		glcd_draw_line(x -1, y, x - 1, y + (_size*7) + 1, _bgcolor);
		glcd_draw_line(x -2, y, x - 2, y + (_size*7) + 1, _bgcolor);
		glcd_draw_line(x -3, y, x - 3, y + (_size*7) + 1, _bgcolor);
	}
